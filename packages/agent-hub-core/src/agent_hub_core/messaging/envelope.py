...
* `payload` must follow the same “no secrets” rule as `JobCreate` so logs, DLQs, and
  ...
  `agent_hub_core.schemas.job.JobCreate.reject_secret_like_keys`.
"""
...
class JobQueueEnvelope(BaseModel):
    ...
    * **`tenant_id`**, **`job_type`**, **`agent_id`**, **`payload`** — Convenience copies
    ...
    job_type: str = Field(
        min_length=1,
        max_length=64,
        description="Worker routing key; same string family as `Job.job_type` / `agent_hub_core.domain.enums.JobType`.",
    )
    ...
    def from_committed_job(
        ...
        *,
        ...
        importable without pulling in ORM models — shared via `agent_hub_core`.
        """
        return cls(
            job_id=job_id,
            tenant_id=tenant_id,
