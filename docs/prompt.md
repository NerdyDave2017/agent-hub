...
- **Infra sequencing (from project plan)**: **Local-first T0** — prove `hub → SQS → worker → Postgres` in Docker **before** heavy Terraform. Later: **per-service Terraform roots** `infra/backend/` → `infra/worker/` → `infra/agents/...` with `terraform_remote_state` for queue URLs, RDS/SG outputs, etc. CI: **path-filtered** workflows per deployable.
