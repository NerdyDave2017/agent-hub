...
- No FastAPI routes—only table shapes and relationships. Runtime DB access lives in
  `core/database.py`; configuration in `core/settings.py`.

Migrations
----------
Schema changes are applied with **Alembic** under `db/migrations/` (see `env.py` and
...
Enums
-----
String enums live in **`domain.enums`** (shared with schemas and services). This file
...
from domain.enums import AgentStatus, AgentType, DeploymentStatus, JobStatus
...
class Job(Base):
    ...
    )  # string discriminator for worker routing; align values with `domain.enums.JobType`
