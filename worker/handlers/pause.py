"""Pause / scale-to-zero jobs — infra implementation deferred."""
...
class AgentPauseHandler(AbstractJobHandler):
    async def execute(self, job: Job, session: AsyncSession) -> None:
        ...
        if not claimed and job.status != JobStatus.running:
            log.warning("pause_skip_unexpected_status", job_id=str(job.id), job_status=job.status.value)
            return

        await complete_job_success(session, job.id, final_step=_DONE)
        await session.refresh(job)
        log.info("pause_stub_complete", job_id=str(job.id))
