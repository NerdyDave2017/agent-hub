"""Deprovision / destroy jobs — infra teardown deferred."""
...
_RUNNING = "stub_destroy"
_DONE = "stub_destroy_complete"
...
class AgentDestroyHandler(AbstractJobHandler):
    async def execute(self, job: Job, session: AsyncSession) -> None:
        ...
        if not claimed and job.status != JobStatus.running:
            log.warning("destroy_skip_unexpected_status", job_id=str(job.id), job_status=job.status.value)
            return

        await complete_job_success(session, job.id, final_step=_DONE)
        await session.refresh(job)
        log.info("destroy_stub_complete", job_id=str(job.id))
